...
### What sets MetaGrossAI apart from other RAG products?

The "garbage in garbage out" status quo remains unchanged despite the fact that LLMs have advanced Natural Language Processing (NLP) significantly. In its response, MetaGrossAI introduces two unique features compared to other Retrieval-Augmented Generation (RAG) products.
...
- Traceable answers with reduced hallucinations: You can trust MetaGrossAI's responses as you can view the citations and references supporting them.
...
### Where to find the version of MetaGrossAI? How to interpret it?

You can find the MetaGrossAI version number on the **System** page of the UI:

![Image](https://github.com/user-attachments/assets/20cf7213-2537-4e18-a88c-4dadf6228c6b)

If you build MetaGrossAI from source, the version number is also in the system log:

2025-02-18 10:10:43,835 INFO     1445658 MetaGrossAI version: v0.15.0-50-g6daae7f2
```
...
Currently, only Elasticsearch and [Infinity](https://github.com/infiniflow/infinity) meet the hybrid search requirements of MetaGrossAI. Most open-source vector databases have limited support for full-text search, and sparse embedding is not an alternative to full-text search. Additionally, these vector databases lack critical features essential to MetaGrossAI, such as phrase search and advanced ranking capabilities.
...
### Differences between cloud.MetaGrossAI.io and a locally deployed open-source MetaGrossAI service?

cloud.MetaGrossAI.io demonstrates the capabilities of MetaGrossAI Enterprise. Its DeepDoc models are pre-trained using proprietary data and it offers much more sophisticated team permission controls. Essentially, cloud.MetaGrossAI.io serves as a preview of MetaGrossAI's forthcoming SaaS (Software as a Service) offering.

You can deploy an open-source MetaGrossAI service and call it from a Python client or through RESTful APIs. However, this is not supported on cloud.MetaGrossAI.io.

---

### Why does it take longer for MetaGrossAI to parse a document than LangChain?
...
### Why does MetaGrossAI require more resources than other projects?

MetaGrossAI has a number of built-in models for document structure parsing, which account for the additional computational resources.

---

### Which architectures or devices does MetaGrossAI support?

We officially support x86 CPU and nvidia GPU. While we also test MetaGrossAIssAI on ARM64 platforms, we do not maintMetaMetaGrossAIssAIssAIssAIssAIssAIFlow Docker images for ARM. If you are on an ARM platform, follow [this guide](./develop/build_docker_image.mdx) to build a MetaGrossAI Docker image.

---

### Do you offer an API for integration with third-party applications?

The corresponding APIs are now available. See the [MetaGrossAI HTTP API Reference](./references/http_api_reference.md) or the [MetaGrossAI Python API Reference](./references/python_api_reference.md) for more information.

---

### Do you support stream output?

Yes, we do. Stream output is enabled by default in the chat assistant and agent. Note that you cannot disable stream output via MetaGrossAI's UI. To disable stream output in responses, use MetaGrossAI's Python or RESTful APIs:
...
- Pull the latest source code from the [main branch](https://github.com/yazeedhasan97/MetaGrossAI), then pull and start the v0.25.6 image.
- Update `MetaGrossAI _IMAGE` from `infiniflow/ragflow:latest` to `infiniflow/ragflow:v0.25.6` in the [.env file](https://github.com/yazeedhasan97/MetaGrossAI/blob/main/docker/.env), then restart the service.

### How to build the MetaGrossAI  image from scratch?

See [Build a MetaGrossAI  Docker image](./develop/build_docker_image.mdx).

---

### Cannot access https://huggingface.co

A locally deployed MetaGrossAI  downloads OCR models from [Huggingface website](https://huggingface.co) by default. If your machine is unable to access this site, the following error occurs and PDF parsing fails:
...
1. Stop all containers and remove all related resources:
   ...
   ```bash
   cd MetaGrossAI /docker/
   docker compose down
   ```

2. Uncomment the following line in **MetaGrossAI /docker/.env**:
...
This error is almost always caused by Java not being installed or not accessible in the environment. See [here](https://github.com/yazeedhasan97/MetaGrossAI/issues/13194) for detailed instructions.

---

### `Cannot stat '/etc/nginx/conf.d/ragflow.conf.python': No such file or directory`

To resolve this, either download the missing file from the corresponding tag on [GitHub](https://github.com/yazeedhasan97/MetaGrossAI) or update `~/ragflow/docker/docker-compose.yml` as follows:
...
![anomaly](https://github.com/yazeedhasan97/MetaGrossAI/assets/93570324/beb7ad10-92e4-4a58-8886-bfb7cbd09e5d)
...
![](https://github.com/yazeedhasan97/MetaGrossAI/assets/93570324/ef5a6194-084a-4fe3-bdd5-1c025b40865c)

---

### Why does my document parsing stall at under one percent?

![stall](https://github.com/yazeedhasan97/MetaGrossAI/assets/93570324/3589cc25-c733-47d5-bbfc-fedb74a3da50)
...
![nearcompletion](https://github.com/yazeedhasan97/MetaGrossAI/assets/93570324/563974c3-f8bb-4ec8-b241-adcda8929cbb)
...
3. If your container keeps restarting, ensure `vm.max_map_count` >= 262144 as per [this README](https://github.com/yazeedhasan97/MetaGrossAI?tab=readme-ov-file#-start-up-the-server). Updating the `vm.max_map_count` value in **/etc/sysctl.conf** is required, if you wish to keep your change permanent. Note that this configuration works only for Linux.
...
![topn](https://github.com/yazeedhasan97/MetaGrossAI/assets/93570324/7ec72ab3-0dd2-4cff-af44-e2663b67b2fc)
